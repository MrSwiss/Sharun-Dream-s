...
void C_DEL_ITEM_f(player_t *player)
{
	...
	SQL_Query_Slow2("UPDATE `character_items` SET `item_count` = '%i' WHERE `owner_id` = '%i' AND `object_id` = '%i';", player->Item_slot[slot].count, player->Character_Id, player->Item_slot[slot].id);
	if (player->Item_slot[slot].count <= 0) {
		player->Item_slot[slot].item = NULL;
		SQL_Query_Slow2("DELETE FROM `character_items` WHERE `owner_id` = '%i' AND `object_id` = '%i';", player->Character_Id, player->Item_slot[slot].id);
	}
